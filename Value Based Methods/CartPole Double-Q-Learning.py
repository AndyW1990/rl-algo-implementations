...
class DQAgent:
    ...
    def __init__(self, alpha=0.1, gamma=0.9, epsilon=1.0):
        ...
        self.scores = []
        self.Qa = np.zeros([11,11,11,11,2])
        self.Qb = np.zeros([11,11,11,11,2])        
    ...
    def choose_action(self, obs, n_actions=2):
        ...
        if np.random.random() < self.epsilon:
            ...
            a_values = [self.Qa[state + (a,)] for a in range(n_actions)]
            b_values = [self.Qb[state + (b,)] for b in range(n_actions)]
            action_values = [a + b for a, b in zip(a_values, b_values)]
...
#                    self.env.CartPole.render()
                ...
                if np.random.random() >= 0.5:
                    a_ = np.argmax( [self.Qa[s_+(actions,)] for actions in range(2)])
                    sa_ = s_ + (a_,)
                    self.Qa[sa] += self.alpha*(reward + self.gamma*self.Qa[sa_] - self.Qa[sa])
                else:
                    a_ = np.argmax( [self.Qa[s_+(actions,)] for actions in range(2)])
                    sa_ = s_ + (a_,)
                    self.Qb[sa] += self.alpha*(reward + self.gamma*self.Qb[sa_] - self.Qb[sa])
