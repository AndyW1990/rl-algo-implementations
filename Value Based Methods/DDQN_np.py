...
class Agent():
    def __init__(self, lr, gamma, n_actions, epsilon, batch_size, 
                  ...
                  mem_size=1000000, replace_target=100, fname='weights'):
        ...
        self.q_targ = Regression_Network(input_dims, n_actions, (256,256), lr)
    ...
    def learn(self):
        if self.memory.mem_cntr > self.batch_size:
            ...
            #predic current and next states with evaluation network
            q_eval = self.q_eval.model_fwd(states)
            q_next = self.q_eval.model_fwd(states_)
            
            #copy the current state q values to update
            q_target = np.copy(q_eval)

            #set indexes for batch
            batch_index = np.arange(self.batch_size, dtype=np.int32)
         
            #update the target
            q_target[actions, batch_index] = rewards + \
                        self.gamma * np.max(q_next, axis=0)*dones
                        
            #train evaluation network with current state and new target value
            losses = self.q_eval.train(states, q_target, iterations=5)
    ...
    def update_network(self):  
            self.q_targ.parameters = self.q_eval.parameters.copy()
        
    def save_weights(self):
        with open(self.fname + '.pickle', 'wb') as handle:
            pickle.dump(self.q_eval.parameters, handle, 
